 ...
 * relative default "config". Used for runtime-mutable state (settings.json,
 * telemetry_*.json, tool_spools.json, custom_images/, etc.). On read-only
 * baselines (Yocto squashfs) HELIX_CONFIG_DIR points at a writable
 * partition (e.g. /etc/klipper/config/helixscreen).
 */
