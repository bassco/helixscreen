...
std::vector<uint8_t> BrotherQLPrinter::build_raster_commands(const LabelBitmap& bitmap,
    ...
    // 3. Set media and quality — ESC i z + 10 bytes
    ...
    // 5. Set cut-every = 1 label
    cmd.push_back(0x1B);
    cmd.push_back(0x69);
    cmd.push_back(0x41);
    cmd.push_back(0x01);

    // 6. Set margins = 0 (no margins for die-cut)
    cmd.push_back(0x1B);
    cmd.push_back(0x69);
    cmd.push_back(0x64);
    cmd.push_back(0x00);
    ...
    // 7. Raster data rows
    // Calculate the right-alignment offset for narrow labels within the 90-byte row
    ...
    for (int y = 0; y < bitmap.height(); y++) {
        ...
        // Check if row is all white (all zeros in bitmap = white)
        bool all_white = true;
        ...
        if (all_white) {
            ...
            // Copy actual pixel data, clamped to label width
            int copy_bytes = std::min(row_bytes, label_byte_width);
            cmd.insert(cmd.end(), row, row + copy_bytes);

            // Right padding if bitmap row is narrower than label byte width
